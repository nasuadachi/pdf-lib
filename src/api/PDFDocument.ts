...
export default class PDFDocument {
  ...
  /**
   * Dispose this document and release references to its internal object graph.
   *
   * This is intended for one-shot generation workflows where the document will
   * not be edited, queried, or saved again after the generated bytes have been
   * handed off to the host environment.
   */
  dispose(): void {
    if (this.disposed) return;

    this.fontkit = undefined;
  ...
  private assertNotDisposed(): void {
    if (this.disposed) throw new PDFDocumentDisposedError();
  }
